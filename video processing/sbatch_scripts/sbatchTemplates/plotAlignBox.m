...
file_dir=[pwd '/aligned/'];
if ~isdir(file_dir)
    errorMessage = sprintf('Error: The following folder does not exist:\n%s', file_dir)
    return;
end
% Get a list of all files in the folder with the desired file name pattern.
filePattern = fullfile(file_dir, '*aligned_info.mat'); % Change to whatever pattern you need.
...
for k = 1 : length(theFiles)
    ...
    fullFileName = fullfile(file_dir, baseFileName);
    load(fullFileName);
    c = linspace(1,10,length(mouseInfo.centroidsF));
    figure
    scatter(mouseInfo.centroidsF(:,1),mouseInfo.centroidsF(:,2), 2, c , 'filled')
